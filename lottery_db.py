...
.tab{display:inline-flex;align-items:center;color:#8c9dba;position:relative}
.tab .tico{opacity:.5;transition:opacity .12s}
.tab:hover{color:#dbe4f5}
.tab:hover .tico{opacity:.85}
...
.lfoot{display:flex;align-items:center;gap:8px;padding-top:9px;
 border-top:1px solid var(--line)}
.lfoot .dt{font-size:14px;color:var(--ink);font-weight:700}
.balls.big{display:flex;flex-wrap:nowrap;align-items:center;gap:8px}
.balls.big .plus{color:var(--mute);font-weight:700;font-size:15px;margin:0 -1px}
.balls.big.n6 .ball{width:42px;height:42px;font-size:17px}
...
.tab.on{box-shadow:inset 0 -3px 0 var(--accent)}
  ...
  if(last){
    H+=`<div class="now" style="--gtint:${G.tint}"><div class="topgrid">
      <div class="lead">
        ...
        <div class="balls big n${last[2].length + (last[3]!==null?1:0)}">${
          last[2].map(x=>`<span class="ball">${ballTxt(G,x)}</span>`).join("")}${
